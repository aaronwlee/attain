...
export const security = (options?: SecurityProps) => {
  ...
  return function security(req: Request, res: Response) {
    ...
    if (DNSPrefetchControl) {
      typeof DNSPrefetchControl === "boolean"
        ? dnsPrefetchControl()(req, res)
        : dnsPrefetchControl(DNSPrefetchControl)(req, res);
    }

    if (frameguard) {
      typeof frameguard === "boolean"
        ? frameGuard()(req, res)
        : frameGuard(frameguard)(req, res);
    }
  };
};
