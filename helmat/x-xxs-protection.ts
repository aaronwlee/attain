...
const doesUserAgentMatchOldInternetExplorer = (userAgent: string | null | undefined): boolean => {
  if (!userAgent) {
    return false;
  }
...
export const xXssProtection = (options: XXssProtectionOptions = {}) => {
  ...
  return (req: Request, res: Response) => {
    const value = doesUserAgentMatchOldInternetExplorer(req.headers.get('user-agent')) ? '0' : headerValue;
